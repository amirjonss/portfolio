...
**Яндекс.Метрика** — счётчик `112551548` (переопределяется `NUXT_PUBLIC_YM_ID`,
пустое значение выключает). Включены вебвизор, карта кликов, точный отказ.

- Загрузчик `tag.js` и `noscript` — в `<head>` статичной сборки (`nuxt.config.ts`)
- `init` и **хиты на смену маршрута** — `app/plugins/analytics.client.ts`.
  Без хитов статический SPA засчитал бы только первую страницу
- Цели — `useAnalytics().goal()`; вызовы стоят в hero, карточках проектов,
  контактах, переключателе языка и на `/cv`

В dev ничего не грузится. Проверить локально: `npm run generate`, поднять
`.output/public`, в DevTools → Network фильтр `mc.yandex`.

**Цели создать в интерфейсе Метрики** (Цели → Добавить → JavaScript-событие,
идентификатор = имя):

| Цель | Когда |
|---|---|
| `cv_pdf` | скачали PDF (параметр `place`: hero / contact / cv) |
| `demo_open` | открыли демо проекта (`project`) |
| `contact_click` | клик по контакту (`channel`: email / github / telegram / linkedin) |
| `lang_switch` | переключили язык (`to`) |
| `scroll_projects`, `scroll_contact` | долистали до секции |

Из них строится воронка: зашёл → проекты → демо → резюме → контакт.

**Cloudflare Web Analytics** — вторая, без cookies: страхует цифры, если
Метрику блокируют. `NUXT_PUBLIC_CF_BEACON_TOKEN` в `.env`; пусто — не подключается.
